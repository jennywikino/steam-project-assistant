...
- README 已更新为可独立阅读的作品集说明页。
- 新增 `docs/portfolio/README.md`，约定六张脱敏截图的命名、内容和提交要求。
- 核心工作流明确为：项目发现 Feed → 项目导入 → 候选池管理 → 基础资料补全 → 一键项目画像 → 报告导出。
- 作品集说明覆盖项目定位、核心功能、技术栈、当前限制和后续方向。
- 未改动候选池数据结构、Steam 导入逻辑或一键项目画像核心逻辑。

适合放入简历的描述：使用 Python、Streamlit、Pandas 与 Playwright 搭建独立游戏发行选品工作台，将 Steam 项目发现、资料补全、候选池筛选、单项目画像和报告导出整理为可复用的本地工作流。

## 当前已完成 V0.8.5-P8-history-export-cleanup
...
- 项目名称：Steam 项目初筛助手
- 当前版本：V0.8.5-P7-2 一键项目画像信息顺序与错误展示修正
...
- 项目名称：Steam 项目初筛助手
- 当前版本：V0.8.5-P6-5-hotfix 候选池补资料队列与导航修复
- 当前定位：面向独立游戏发行/运营场景的本地 Steam 项目筛选工作台

## 当前已完成 V0.8.5-P6-5-hotfix
...
- 候选池主表新增可编辑选择列，批量选择和手工勾选统一写入同一个 AppID 集合。
- 选中项目处理台紧跟主表，可在已选候选间切换并查看业务快照。
- 单项目支持待试玩、值得联系、暂缓、放弃、待补资料快捷流转，保存时保留未编辑字段。
- 手动编辑区保留阶段、优先级、下一步、备注、放弃原因和归档状态。
- 已选候选可批量修改阶段、优先级、下一步和归档状态，也可使用五个快捷批量动作。
- 批量操作必须确认，不覆盖备注或放弃原因，并在操作区显示成功、失败和更新时间。
- 规则建议降级为本地辅助入口，不再使用容易误解为 AI 判断的按钮文案。
- 普通导出仅显示当前筛选 CSV / Excel 浏览器下载，中文表头不含内部调试字段。
- 旧版导出默认收起，避免打断主表到处理台的操作路径。

## 当前已完成 V0.8.5-P6-1

- 项目候选池顶部新增候选总览和最近 5 批入池记录，支持按批次 AppID 聚焦列表。
- 最近入池优先读取 action log，并用候选池来源、时间和 batch_id 字段补充历史记录。
- 候选池 schema 新增入池时间、来源页面、批次、导入方式及中文、图片、内容类型、好评率字段。
- 新导入记录保留来源和批次，旧数据缺失字段时不报错、不覆盖人工字段。
- 批量补全实际调用 Steam 基础信息和评测缓存接口，跳过无 AppID 项目并只写入空字段。
- 补全后立即刷新候选池，并持久显示成功、缓存命中、失败和跳过摘要。
- 默认主表收口为 17 个工作流字段，完整度、缺失项和规则建议仅在完整字段模式显示。
- 搜索支持来源文本，高级筛选默认收起，Steam 和项目画像使用可点击入口。

## 当前已完成 V0.8.5-P5-hotfix2

- SteamDB 粘贴解析支持换行、英文/中文逗号、英文/中文分号和 Tab 分隔。
- 只有整段为纯 AppID 列表时才允许空格分隔，普通游戏名继续作为单条无法解析记录。
- 无法解析项不会进入可入池结果、候选池发送或用户版 CSV。
- 发送候选池反馈紧跟发送操作栏显示，早于已选预览和当前项目快照。
- 用户版 CSV 使用固定中文字段和 UTF-8 BOM，隐藏 source_row、parse_notes、source_notes 等内部字段。
- 普通模式仅显示“导出结果”浏览器下载，调试模式才提供完整内部表和 exports 写入。
- 快照更多字段只保留已有 SteamDB 数值，不显示解析备注和原始工程字段。

## 当前已完成 V0.8.5-P5-hotfix

- SteamDB 粘贴导入只保留为文本解析备用入口，不提供游戏名搜索或自动访问 SteamDB 的暗示。
- 解析结果按是否存在 AppID 分流；无效行只进入“无法解析记录”，不会参与选择、快照或发送。
- 主表和已选项目预览使用同一套业务字段，弱 SteamDB 指标不再占据主视线。
- 项目快照默认仅展示头图、项目身份、发行判断、内容、试玩、中文、类型和价格。
- Followers、Reviews、Peak、Rating 与解析备注仅在更多字段折叠区按需显示。
- 候选池发送结果统一为四项统计和带候选池状态的发送明细。
- CSV 通过浏览器下载；只有调试模式才允许额外写入 exports。
- 榜单观察记录在普通模式隐藏。

## 当前已完成 V0.8.5-P5-pre

- SteamDB 粘贴导入明确为本地文本解析入口，不承担游戏名搜索或 SteamDB 自动抓取。
- 页面提供粘贴、解析、补全、选择和发送候选池的顺序化主流程。
- 补全基础信息前置，并继续复用现有 Steam 基础信息补全能力，不新增请求源。
- 普通模式移除“全选高可信”，批量选择只围绕已识别 AppID 的可入池项目。
- 选择状态统一为 session_state AppID 集合，表格 checkbox、统计、快照和发送使用同一状态。
- 解析表、三项统计、项目快照和候选池发送结果对齐 Steam 搜索导入与页面采集。
- CSV 导出和完整解析字段收进默认折叠的“导出 / 调试”区域。

## 当前已完成 V0.8.5-P4

- Steam 页面采集明确使用工具专用浏览器，不读取普通 Chrome / Edge 的 Steam 登录状态。
- 页面定位为列表页采集工具，推荐抓取搜索、榜单、标签、分类和活动页具体列表区域。
- 单个游戏不再提供页面抓取入口，统一提示复制 Steam URL 或游戏名到项目画像。
- 抓取结果自动保存并按 AppID 合并或更新，主界面不再提供保存和手动去重按钮。
- 清空本次采集收进默认折叠的危险操作区，并要求勾选确认。
- 主表统一为序号、选择、头图、游戏信息、采集日期、内容语义、Steam 和项目画像入口。
- 本地筛选、批量选择、三项统计和可切换项目快照使用统一 AppID 选择集合。
- 页面列表无法识别时提供搜索页、榜单页、单 App 和新品节列表区域的分流建议。

## 当前已完成 V0.8.5-P3-hotfix

- Steam 页面采集环境检测以当前 Streamlit 进程的 `sys.executable` 为准。
- 不再仅检查包、命令或浏览器目录；会实际启动并关闭 headless Chromium。
- 诊断结果包含当前 Python、Playwright 安装状态和版本、Chromium 启动状态及具体错误。
- 自动安装使用当前 Python 执行模块命令，并在完成后立即重新检测，不缓存失败结果。

## 当前已完成 V0.8.5-P3

- Steam 页面采集在 Playwright 或可用浏览器缺失时显示面向普通用户的安装说明。
- 页面提供自动安装入口，严格通过项目 `.venv` 执行 pip 和 Playwright 模块命令。
- 自动安装捕获 stdout / stderr；失败时显示错误并引导运行 BAT。
- 根目录新增“安装采集环境.bat”，可独立安装 Playwright Python 包和 Chromium。
- 已安装环境继续进入原 Steam 页面采集流程，不改采集核心逻辑。

## 当前已完成 V0.8.5-P2-hotfix-3

- Steam 搜索导入当前项目快照不再显示好评率、评论数和评价。
- 搜索导入页专注批量初筛；评测详情统一后移到候选池或项目画像。
- 快照继续保留工具发现时间、发售、内容类型、试玩、中文、类型、价格和三个主动作。

## 当前已完成 V0.8.5-P2-hotfix-2

- Steam 搜索导入主表删除“首次发现”，工具发现时间仅在快照中以完整时间显示。
- 主表删除“好评率”，不在批量初筛表中放置大量空评测字段，也不触发批量 appreviews。
- 主表收口为序号、选择、头图、游戏名、AppID、开发、发行、发售、试玩、中文、类型、Steam、项目画像。
- 页面明确复杂语言、价格、标签、类型、人数和功能筛选应在 Steam 搜索页完成，再复制 `/search/` URL。
- 工具内继续保留发行工作流常用的本地筛选。

## 当前已完成 V0.8.5-P2-hotfix

- 搜索导入只接受 Steam 商店 `/search/` 结果页，自定义 URL 在请求前严格校验。
- Next Fest、Explore/New、单个 App、Bundle、Sub、开发商/发行商页和外部域名不会触发抓取或默认页 fallback。
- 不支持 URL 不会改变 offset、当前导入结果或选择集合，并提供 Steam 页面采集 / 查查 / 项目画像分流建议。
- 普通界面只保留来源、排序、批次和三个主操作；请求数量、间隔和下一批起点放入高级设置。
- 主表压缩为 15 列，删除内容 Unknown，首次发现保留年份，好评率缺失显示 `—`。
- 排除非游戏内容仅作用于明确识别的 DLC、Demo、Tool、OST、Artbook 和 Software。

## 当前已完成 V0.8.5-P2

- Steam 搜索导入主表控制为 16 列，字段收口为内容、试玩、中文、首次发现和评测。
- 内容类型与是否有试玩分离，支持识别游戏、Demo、DLC、Tool、OST、Artbook、Software、Video、Playtest 和 Unknown。
- 简中与繁中统一纳入中文支持判断，具体中文类型在项目快照显示。
- 主表日期压缩显示，完整首次发现时间转移到快照。
- Steam 搜索设置分为源端筛选和本地结果筛选，不再展示大面积无效 disabled 控件。
- offset 保存在 session_state，支持查看当前批次并抓取下一批。
- action log 与候选池 AppID 用于隐藏已处理 / 只看未处理项目。
- 项目快照仅对当前项目复用 12 小时缓存的 Steam appreviews 摘要，不批量请求 50 条。
- Steam 搜索主表删除查查入口；查查结果页项目画像动作合并为单按钮。
- 全局右侧观察浮窗不在本轮实现。

## 当前已完成 V0.8.5-P1

- Steam 搜索导入页增加操作流程、来源说明和 Steam 官方页面预浏览入口。
- “抓取 AppID”改为“抓取游戏”，补全动作明确说明覆盖字段。
- 增加 Steam 搜索排序依据；未接入的高级筛选项禁用并标明状态。
- 数据管理只保留去除重复游戏和带确认的清空本次导入。
- 快速筛选、批量选择、单行勾选统一使用 session_state AppID 集合。
- 主表增加序号、头图、商店上架/首次发现时间，并提供 Steam、查查、项目画像三个入口。
- 当前项目快照支持切换已选项目，无已选项目时可预览当前筛选前 50 条。
- 主表、快照和发送结果不再展示导入建议、建议理由、下一步动作。
- 已选统计只保留一组，查看已选项目不再因重复点击而消失。
- 未新增 Steam 请求源，未修改候选池、查查、项目画像和其他导入模式核心逻辑。
