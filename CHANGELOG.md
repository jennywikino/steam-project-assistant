...
- 修复已上线 Early Access 项目因旧 Coming Soon 文本被误归入“未上线/TBA”。
- 已过去或当天的发布日期、评论数、在线人数、可购买/已发售标记作为已上线证据，优先于旧未上线文本。
- 已上线 Early Access 卡片显示“已发售 / EA”和“抢先体验”标签。
- Coming Soon、TBA、未定年份、空日期及无上线证据的未来日期继续保留在未上线筛选。
- EMPULSE / AppID 4323990 作为回归样例。

## V0.9.3-P0f-hotfix

- 删除首页重复的快捷入口栏；项目导入、候选池和项目画像页面保持不变。
- 将“重新读取本地数据”移入高级刷新区，把“刷新（需联网 Steam）”移动到观察窗口控件旁。
- 将“时间口径”改为“按哪个时间筛选”，选项缩短为自动、发现/读取时间、入池/导入时间和最近补全时间。
- 首页数据范围说明压缩为一行，详细解释收纳到“关于首页数据范围”折叠区，并明确首页不是 Steam 全量库。
- 卡片统一显示“来源”和“观察时间”；候选入池时间作为补充字段，评论数未知时显示“未确认”。
- 未上线/TBA 空状态明确限定为当前观察窗口和本工具已采集范围。

## V0.9.3-P0e-hotfix

- 首页 Feed 新增观察窗口：今天、最近 7 天、最近 30 天、全部本地记录、自定义日期范围；默认最近 7 天。
- 新增时间口径：图文源读取时间、入池/导入/采集时间、最近补全时间、全部可用时间自动选择。
- canonical cards 先按观察窗口与时间口径过滤，再执行首页五个筛选；有限日期窗口不会用无时间项目补位。
- 页面显示观察窗口起止日期、时间口径、筛选数量和基础信息补全数量。
- 补充 TBA 当前状态语义和窗口空状态说明；历史状态变化仍需后续快照功能。

## V0.9.3-P0d-hotfix

- 首页 Feed 筛选收口为：全部、最新采集、未上线/TBA、有 Demo/可试玩、无发行/自发行。
- 删除首页“潜力观察”“竞品参考”和“热门/评论多”筛选入口及对应过滤分支，不影响候选池字段和页面。
- 除“全部”外的筛选只处理当前最新 Steam 图文源批次；无匹配时显示空状态，不使用旧候选池、搜索导入或页面采集项目补位。
- 刷新图文源后筛选重置为“全部”，清理分页及派生展示状态后重新计算。
- 未知 Demo、publisher 和 developer 不会被当作无 Demo 或自发行。

## V0.9.3-P0c-hotfix

- Steam 图文源刷新成功后，对最新前 30 个唯一 AppID 复用现有 appdetails 和 review cache helper 做轻量补全。
- canonical Feed 在筛选前只读现有详情/评论缓存并合并图文源卡片，使新项目可以进入 Demo、自发行、热门、潜力和竞品筛选。
- 图文源时间、补全时间和候选池更新时间分别保存在 `source_time/feed_fetched_at`、`enriched_at`、`candidate_updated_at`。
- 未补全的 publisher、Demo、评论和发售状态保持未知，不再将未知 publisher 误判为自发行。
- 首页 Feed 显示基础信息已补全与待补资料数量。

## V0.9.3-P0b-hotfix

- 确认首页八个筛选统一从 canonical Feed cards 派生，不再按筛选重新读取旧来源。
- “最新采集”包含全部 canonical cards，并按采集、创建、更新、图文源、来源和导入时间排序；无时间项目排在末尾。
- 手动、自动和高级 Steam 图文源刷新复用同一 invalidation helper，成功后清理 Feed cards、筛选结果、可见结果和分页展示状态。
- Feed 区明确显示当前筛选来自统一首页 Feed。

## V0.9.3-P0a-hotfix

- 首页先构建唯一 canonical Feed，再由全部八个筛选项统一过滤和排序。
- “最新采集”改为统一 Feed 中存在来源时间的项目，包含新刷新的 Steam 图文源。
- 顶部手动刷新、24 小时自动刷新和高级强刷复用同一刷新 helper；成功后统一清理 Feed 分页与派生展示状态。
- Feed 筛选区显示统一 Feed 总数、当前筛选项目数和已显示数量。

## V0.9.3-P0-hotfix

- 首页顶部新增“刷新 Steam 图文源（联网）”，并区分页面读取时间与图文源刷新时间。
- 图文源缓存不存在或超过 24 小时时，每个 Streamlit 会话自动尝试联网刷新一次；失败继续使用旧缓存。
- Steam appdetails 完整保留多开发商 / 多发行商；刷新与多来源合并时优先保留公司数量更多的值，并记录来源差异。
- 已有非开发商发行方的项目不再标记为自发行，自动理由会注明检测到的发行商。
